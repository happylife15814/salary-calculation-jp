...
給与計算プログラム - 2026年6月分
変更点:
- シフト家事時間.xlsxのA列〜D列（4列、36行まで）を読み込む
- R列「ミーティング」でMTG判定（シフトMTG.xlsx不要）
- 鍵預かりお礼金リストはC列から読み込む
- A5 = 家事時間 + 日常清掃 + 研修 + MTG + 鍵預かり + 交通費
"""
...
OUTPUT_DIR = '/home/ubuntu/salary_output_jun'
TEMPLATE_FILE = os.path.join(UPLOAD_DIR, '給与明細フォーマット_blank.xlsx')
KINTAI_FILE = os.path.join(UPLOAD_DIR, '【2026年06月】勤務表.xlsx')
...
def load_wages():
    ...
    # 家事時間: A列〜D列（4列）、36行まで読み込む
    wb = load_workbook(KAJI_MASTER)
    ws = wb.active
    for row in ws.iter_rows(min_row=1, max_row=36, min_col=1, max_col=4, values_only=True):
        for cell in row:
            if cell:
                n = normalize(cell)
                if n:
                    wages['kaji'].add(n)
    wb.close()
    print(f'家事時間シフト数: {len(wages["kaji"])}件')
...
def write_output(name, records, heikin_info, transport_fees, kagi_list):
    ...
    # A5: 総支給額（家事時間 + 日常清掃 + 研修 + MTG + 鍵預かり + 交通費）
